...
if __name__ == '__main__':
    ...
    with open(f"{lf_folder}/metrics.txt", "a") as f:
        if isinstance(result, tuple) and len(result) == 2:
            score, _ = result
            print(f"CAPTURE score turn2: {score:.4f}", file=f)
            ...
            print(f"CAPTURE-like object precision turn2: {object_precision:.4f}", file=f)
            print(f"CAPTURE-like object recall turn2: {object_recall:.4f}", file=f)
            print(f"CAPTURE-like object f1 turn2: {object_f1:.4f}", file=f)
            print(f"CAPTURE-like attribute precision turn2: {attribute_precision:.4f}", file=f)
            print(f"CAPTURE-like attribute recall turn2: {attribute_recall:.4f}", file=f)
            print(f"CAPTURE-like attribute f1 turn2: {attribute_f1:.4f}", file=f)
        else:
            print("CAPTURE output format not recognized for turn2.", file=f)
