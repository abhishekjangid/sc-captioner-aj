...
class CAPTURE:
    def __init__(
        ...
        if self.soft_matching:
            if torch.cuda.is_available():
                ...
                device = "cpu"
            self.text_encoder = SentenceTransformer("sentence-transformers/all-mpnet-base-v2").to(device).eval()
    ...
    def process_samples_multiprocessing(self, partitioned_data, desc="parsing"):
        ...
        if torch.cuda.device_count() > 0:
            with multiprocessing.Pool(processes=torch.cuda.device_count()) as pool:
                ...
                for idx, this_partitioned_data in enumerate(partitioned_data):
                    future = pool.apply_async(
                        self.parse_samples,
                        args=(this_partitioned_data, torch.device(f'cuda:{idx}'), desc),
                    )
            ...
            # macOS/CPU fallback: parse in a single process to avoid CUDA-only pool sizing.
            all_parsed = self.parse_samples(partitioned_data[0], torch.device('cpu'), desc)
    ...
    def compute_score(self, gts, res, prev_gt_parsed=None, prev_cand_parsed=None, return_parse_results=False, extra_objects=None, extra_attributes=None):
        ...
        for sample_key in tqdm.tqdm(gt_parsed.keys(), desc="computing score"):
            ...
            if extra_objects is None:
                sample_extra_objects = list(sample_gt_parsed[0][0])
            else:
                sample_extra_objects = extra_objects.get(sample_key, [])

            if extra_attributes is None:
                sample_extra_attributes = {k: list(v) for k, v in sample_gt_parsed[0][1].items()}
            else:
                sample_extra_attributes = extra_attributes.get(sample_key, {})
